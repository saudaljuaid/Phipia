...
#ifndef PYRENIS_ACPI_H
#define PYRENIS_ACPI_H
...
/*
 * A Pyrenis early-boot policy bound on how many configuration windows firmware
 * may declare, not an architectural one. Every machine Pyrenis is tested on
...
/*
 * The fixed ACPI description table, reduced to the one facility Pyrenis uses
...
/*
 * Pyrenis early-boot policy bounds on firmware-declared topology. They keep the
