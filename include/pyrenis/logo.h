...
#ifndef PYRENIS_LOGO_H
#define PYRENIS_LOGO_H
 ...
 *
 * This is the first thing in Pyrenis that is not C, and the choice is argued in
...
int32_t pyrenis_logo_self_test(void);

/* How many bytes the built-in image occupies. */
size_t pyrenis_logo_size(void);

/* Read the declared size without decoding. Both pointers must be non-null. */
int32_t pyrenis_logo_geometry(uint32_t *width, uint32_t *height);
