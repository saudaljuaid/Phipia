...
#ifndef SAPOTE_ACPI_H
#define SAPOTE_ACPI_H
...
/*
 * A Sapote early-boot policy bound on how many configuration windows firmware
 * may declare, not an architectural one. Every machine Sapote is tested on
...
/*
 * The fixed ACPI description table, reduced to the one facility Sapote uses
...
/*
 * Sapote early-boot policy bounds on firmware-declared topology. They keep the
