...
#ifndef SAPOTE_LOGO_H
#define SAPOTE_LOGO_H
 ...
 *
 * This is the first thing in Sapote that is not C, and the choice is argued in
...
int32_t sapote_logo_self_test(void);

/* How many bytes the built-in image occupies. */
size_t sapote_logo_size(void);

/* Read the declared size without decoding. Both pointers must be non-null. */
int32_t sapote_logo_geometry(uint32_t *width, uint32_t *height);
