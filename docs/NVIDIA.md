...
Run against it, with the boot register pinned to a Turing encoding and the ROM
image generated by the independent Python record:

```
ST NVIDIA driver 0 NVIDIA GPU master control bound 0x164000A1/0x0 reads 2 writes 0
ST NVIDIA driver 1 NVIDIA GPU configuration mirror bound 0x1B8010DE/0x1B8010DE reads 2 writes 0
ST NVIDIA driver 2 NVIDIA GPU timer bound 0xB995B887/0x204920 reads 6 writes 0
ST NVIDIA driver 3 NVIDIA GPU video BIOS bound 0x10DE5341/0x400 reads 8194 writes 2
ST NVIDIA driver 4 NVIDIA HD Audio function absent
ST NVIDIA declared 5 present 4 bound 4 controls 14 architecture Turing
   chipset 0x164 reads 8204 writes 2 function present teardown clean census equal
ST PASS nvidia
```

Four drivers bound; the audio one correctly reported absent, because the model
...
- driver 2's second value is the delta: `0x204920` is 2,116,384 ns across a
...
Two controls prove the run is not vacuous:
