...
| --- | --- | --- |
| 1. General load and clean exit | `native` | Loads `NATIVET.MAN` from System and requires `SAPOTE NATIVE PASS` plus the kernel loader/SDK/TLS/thread/FPU completion line. |
| 2. Initial stack | `native` | The C entry code validates exact `argc`, `argv`, three environment entries, terminators, and Sapote auxiliary entries before `SAPOTE STARTUP argc argv environment auxiliary PASS`. |
...
| 7. Stale handle | `native` | Duplicates and closes a file handle, then requires `-ESTALE` for use and repeated close before `SAPOTE STORAGE typed duplicate stale-handle PASS`. |
...
| 15. Keyboard and pointer delivery | `native-canvas` | Hardware injection must produce positive key and pointer counts in a Ring 3 `SAPOTE CANVAS PASS` line. |
