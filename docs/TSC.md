...
The local APIC timer was calibrated against the PIT, which left Pyrenis with one
...
instructions relative to the surrounding code. Every interval Pyrenis measures
...
target that bit is **not** set, and Pyrenis reports it rather than assuming it:
...
moments ago, and it is not yet a time base Pyrenis may trust across long spans or
...
```text
Pyrenis: TSC calibrated at <n> Hz, invariant <yes|no>
Pyrenis: TSC reference established
```
