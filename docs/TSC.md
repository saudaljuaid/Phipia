...
The local APIC timer was calibrated against the PIT, which left Sapote with one
...
instructions relative to the surrounding code. Every interval Sapote measures
...
target that bit is **not** set, and Sapote reports it rather than assuming it:
...
moments ago, and it is not yet a time base Sapote may trust across long spans or
...
```text
Sapote: TSC calibrated at <n> Hz, invariant <yes|no>
Sapote: TSC reference established
```
