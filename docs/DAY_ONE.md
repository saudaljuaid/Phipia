...
Seneri starts on x86_64 through the GNU Multiboot2 protocol. GRUB is a temporary
loader, not part of the kernel architecture. It keeps day one focused on the
first code Seneri owns rather than on disk formats and firmware edge cases.

## Boot invariants

1. `_start` receives the Multiboot2 magic in EAX and the information address in
   EBX while running in 32-bit protected mode.
2. Interrupts and the direction flag are cleared before Seneri touches its own
   state.
...
- Linking has no unresolved symbol and no load segment that is writable and
  executable.
- A clean QEMU boot emits `Seneri OS: day one passed` through COM1 within ten
  seconds.
