...
| `filesystem.c` | 1629 | Typed FAT16 orchestration, CPU/controller ownership, inherited four-read proofs, stable proof, and separate process and BusyBox file seams. |
...
| `paging.c` | 4185 | Four-level page tables, W^X, supervisor mapping intent, one private user hierarchy, PAT ownership, and WB/WC/UC memory types. Read `DEVICE_WINDOWS.md` and `PROCESS_ADDRESS_SPACE.md`. |
...
| `linux_abi.c` | 1619 | One typed BusyBox candidate-to-release lifecycle, Linux initial stack, bounded stdout sink, syscall provenance, exhaustive cleanup injection, census and reverse teardown. |
| `linux_syscall.c`, `arch/x86_64/linux_syscall.S` | 1117 + 151 | Programmed Linux x86-64 SYSCALL MSRs, immediate kernel-stack entry, checked dispatch, controlled before/after-call failure returns and IRETQ return. |
...
| `rust/linux_fat16.rs` | 703 | Safe nine-cluster BusyBox FAT16 chain parser and bounded multi-cluster payload copier. |
| `rust/abi.rs`, `rust/lib.rs` | 721 + 43 | What the two languages promise each other. |

### Proving it

| File | | |
| --- | ---: | --- |
| `test.c` | 5058 | The thirty-eight QEMU scenarios and what each must print. |
