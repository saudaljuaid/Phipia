...
Sapote has sixty-seven source files and sixty-one focused documents. This map points
...
| File | | |
| --- | ---: | --- |
| `kernel.c` | 137 | Reversible console bootstrap, validate/execute/installed-proof boundary, then scenario or shell/UI handoff. |
| `boot_plan.c` | 3099 | The installed descriptors, typed dependency declarations, context population and private stage execution functions. |
| `boot_ledger.c` | 2482 | Pure bounded planning, named refusals, receipts, deterministic fingerprint and installed-ledger verification. |
...
| `filesystem.c` | 1708 | Typed FAT16 orchestration, CPU/controller ownership, inherited four-read proofs, stable proof, and separate process, echo, and uname file seams. |
...
| `paging.c` | 4241 | Four-level page tables, W^X, supervisor mapping intent, one private user hierarchy, exact echo/uname layouts, PAT ownership, and WB/WC/UC memory types. Read `DEVICE_WINDOWS.md` and `PROCESS_ADDRESS_SPACE.md`. |
...
| `linux_abi.c` | 1663 | The inherited typed echo BusyBox lifecycle, Linux initial stack, bounded stdout sink, syscall provenance, exhaustive cleanup injection, census and reverse teardown. |
| `linux_uname.c` | 1701 | The separate uname executable/stack/process/read lifecycle, eleven-page W^X install, 81 cleanup injections, census and reverse teardown. |
| `linux_syscall.c`, `arch/x86_64/linux_syscall.S` | 1814 + 151 | Programmed Linux x86-64 SYSCALL MSRs, immediate kernel-stack entry, checked echo/uname dispatch, non-partial UTS copy-out, controlled call failures and IRETQ return. |
...
| `rust/linux_elf64.rs` | 710 | Safe bounded static BusyBox parser with separate measured echo/uname program-header, W^X, BSS and entry contracts. |
| `rust/linux_fat16.rs` | 793 | Safe separately typed nine-cluster echo and ten-cluster uname FAT16 chains and bounded payload digests. |
| `rust/abi.rs`, `rust/lib.rs` | 926 + 43 | What the two languages promise each other. |

### Proving it

| File | | |
| --- | ---: | --- |
| `test.c` | 5139 | The thirty-nine QEMU scenarios and what each must print. |
...
The canonical descriptor sequence is:
    ...
    -> static BusyBox uname image/UTS foundation
    -> optional installed BusyBox uname proof/outcome
    -> closing proofs
