...
Five commands, in this order, every time.
    ...
    make qemu-tests    # All thirty-one bounded scenarios; time varies by host.
...
## Starting a change
    ...
    git fetch origin
    git switch -c pyrenis-<what-it-does> origin/main

Branch names describe the change, not who made it. `pyrenis-pci-enumeration`,
`pyrenis-pit-retirement`, `pyrenis-monotonic-clock`. Never a tool's name and
never a person's.
...
## Pushing, and the pull request
    ...
    git push -u origin pyrenis-<what-it-does>
...
| Control | Result |
| --- | --- |
| Add one unused variable to `src/kernel/logo.c` and run `make verify`. | Exit status 2. `error: unused variable 'pyrenis_control_unused' [-Werror=unused-variable]`. |
...
2. `include/pyrenis/<subsystem>.h` — the surface, and the comments above each
