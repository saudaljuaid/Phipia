...
## Starting a change
    ...
    git fetch origin
    git switch -c sapote-<what-it-does> origin/main

Branch names describe the change, not who made it. `sapote-pci-enumeration`,
`sapote-pit-retirement`, `sapote-monotonic-clock`. Never a tool's name and
never a person's.
...
## Pushing, and the pull request
    ...
    git push -u origin sapote-<what-it-does>
...
| Control | Result |
| --- | --- |
| Add one unused variable to `src/kernel/logo.c` and run `make verify`. | Exit status 2. `error: unused variable 'sapote_control_unused' [-Werror=unused-variable]`. |
...
2. `include/sapote/<subsystem>.h` — the surface, and the comments above each
