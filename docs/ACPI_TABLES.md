...
Sapote follows the RSDT only for an ACPI 1.0 root and otherwise follows the
...
fit within Sapote's current first-4-GiB identity map, and contain a whole number
...
are Sapote policy bounds, not ACPI architectural limits. They keep every loop
...
access. Before Sapote examines a referenced signature, the address must be
...
The memory-mapped configuration table is the first table Sapote reads whose
...
```text
Sapote: ACPI MADT verified
Sapote: ACPI configuration windows verified
```
...
run and reports `Sapote: ACPI MCFG absent`. The present path is proved by
...
- **Segment groups beyond the first are recorded and otherwise ignored.**
  Nothing in Sapote yet has a second segment to address.
...
topology is proved may Sapote mask the legacy PIC permanently, route a timer
