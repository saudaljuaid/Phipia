...
Pyrenis follows the RSDT only for an ACPI 1.0 root and otherwise follows the
...
fit within Pyrenis's current first-4-GiB identity map, and contain a whole number
...
are Pyrenis policy bounds, not ACPI architectural limits. They keep every loop
...
access. Before Pyrenis examines a referenced signature, the address must be
...
The memory-mapped configuration table is the first table Pyrenis reads whose
...
```text
Pyrenis: ACPI MADT verified
Pyrenis: ACPI configuration windows verified
```
...
run and reports `Pyrenis: ACPI MCFG absent`. The present path is proved by
...
- **Segment groups beyond the first are recorded and otherwise ignored.**
  Nothing in Pyrenis yet has a second segment to address.
...
topology is proved may Pyrenis mask the legacy PIC permanently, route a timer
