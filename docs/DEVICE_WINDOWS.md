...
Sapote now builds one `struct paging_device_windows` after boot has discovered
...
`include/sapote/paging.h` defines three public pieces:
...
page selection. Sapote still neither reads nor programs MTRRs, so bare-metal
...
- The registry covers only windows Sapote currently touches; unowned MMIO holes
