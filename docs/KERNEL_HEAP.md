...
is a Sapote policy bound, like every other one in this kernel, and running into
...
```text
Sapote: heap window 0x0000000400000000 size 16777216 guards 0x00000003FFFFF000 0x0000000401000000
Sapote: heap committed 4096 bytes in 1 pages, live 3
Sapote: kernel heap online
Sapote: heap coalesced to one free block
Sapote: kernel heap established
```
...
- **Not reentrant, and not interrupt-safe.** Growth maps pages in a transaction
  that must not be interrupted, and nothing in Sapote allocates from an
