...
This increment delivers a real interrupt through hardware Pyrenis discovered
...
which one Pyrenis happened to examine first.

Initialization masks every redirection entry on every discovered unit. Firmware
may leave entries unmasked, and none of them are Pyrenis's until Pyrenis programs
them.
...
Only the fields Pyrenis wrote are compared when an entry is read back. Delivery
...
overrides say where it actually lands and how it is wired, and Pyrenis applies
...
high, which is what Pyrenis assumes and nothing more.

Both flag fields reserve one encoding. A reserved encoding describes electricals
Pyrenis cannot program, so it is refused by name rather than read as the nearest
...
link interrupts, and until this increment Pyrenis refused to route them at all.
...
The entry is read back afterwards, comparing only the fields Pyrenis wrote.
...
offset 0x40. Pyrenis sets local APIC SVR bit 12 only when both are present, reads
...
driver owes its hardware, and Pyrenis's one device is no exception: on this route
...
Firmware declares IRQ0 edge triggered, and Pyrenis's production routing honours
...
```text
Pyrenis: I/O APIC id 0 version 0x0000000000000020 entries 24 base GSI 0 directed EOI yes
Pyrenis: local APIC EOI-broadcast suppression unsupported active no
Pyrenis: I/O APIC level route id 0 GSI 2 vector 48 active high acknowledgement broadcast
Pyrenis: I/O APIC level deliveries 8 remote IRR 8 directed EOI 0 in 86602068 ns
Pyrenis: I/O APIC delivered eight level-triggered interrupts
Pyrenis: level-triggered routing established
```
...
back. Pyrenis therefore reports and proves broadcast mode there. Page-table or
