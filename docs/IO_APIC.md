...
This increment delivers a real interrupt through hardware Sapote discovered
...
which one Sapote happened to examine first.

Initialization masks every redirection entry on every discovered unit. Firmware
may leave entries unmasked, and none of them are Sapote's until Sapote programs
them.
...
Only the fields Sapote wrote are compared when an entry is read back. Delivery
...
overrides say where it actually lands and how it is wired, and Sapote applies
...
high, which is what Sapote assumes and nothing more.

Both flag fields reserve one encoding. A reserved encoding describes electricals
Sapote cannot program, so it is refused by name rather than read as the nearest
...
link interrupts, and until this increment Sapote refused to route them at all.
...
The entry is read back afterwards, comparing only the fields Sapote wrote.
...
offset 0x40. Sapote sets local APIC SVR bit 12 only when both are present, reads
...
driver owes its hardware, and Sapote's one device is no exception: on this route
...
Firmware declares IRQ0 edge triggered, and Sapote's production routing honours
...
```text
Sapote: I/O APIC id 0 version 0x0000000000000020 entries 24 base GSI 0 directed EOI yes
Sapote: local APIC EOI-broadcast suppression unsupported active no
Sapote: I/O APIC level route id 0 GSI 2 vector 48 active high acknowledgement broadcast
Sapote: I/O APIC level deliveries 8 remote IRR 8 directed EOI 0 in 86602068 ns
Sapote: I/O APIC delivered eight level-triggered interrupts
Sapote: level-triggered routing established
```
...
back. Sapote therefore reports and proves broadcast mode there. Page-table or
