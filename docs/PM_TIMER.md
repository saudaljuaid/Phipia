...
Seneri had two clocks before this increment, and both were calibrated against
...
frequency the original PC clock chain was built from. Seneri does not measure
...
```text
Seneri OS: PM timer measured 9999874 ns against TSC 20172796 ns
Seneri OS: local APIC timer calibrated at 31299710 counts per second
```
...
- QEMU's local APIC timer counts one tick per 16 ns under divide-by-sixteen, so
  62.5 MHz exactly. Seneri measured 31.3 MHz — half.
...
```text
Seneri OS: local APIC timer calibrated at 62643780 counts per second
Seneri OS: TSC calibrated at 2806852220 Hz, invariant no
Seneri OS: PM timer measured 9999874 ns against TSC 10130453 ns
```
...
```text
Seneri OS: ACPI FADT at 0x0000000007FE1B06 revision 1 flags 0x00000000000080A5
Seneri OS: ACPI PM timer port 0x0000000000000608 width 24 bits address fixed
```

### Which address wins

ACPI 6.6 gives `X_PM_TMR_BLK` precedence: when it holds an address the operating
system can use, `PM_TMR_BLK` must be ignored. Seneri applies that literally.
...
| --- | --- |
| `PM_TIMER_FREQUENCY_HZ` multiplied by four | `Seneri OS PANIC: ACPI PM timer arithmetic self-test failed` |
