...
Sapote now gives exactly that framebuffer span a write-combining page memory
...
eight bytes is one type. Sapote accepts only the architectural encodings:
...
because the bootstrap hierarchy and Sapote's register windows rely on those two
meanings.
...
Before this change, Sapote's RAM leaves also selected entry 0 and its live
register windows selected entry 3. No bootstrap or live mapping selected entry
1. Sapote may therefore replace only byte 1 without changing the meaning of an
...
Sapote neither changes nor claims ownership of firmware MTRRs. PAT and MTRRs
...
to be combined. Sapote does not yet read the MTRRs, so it cannot claim that the
