...
Sapote looked like it enforced W^X. It did not.
...
This is the third time Sapote has found a property that was verified in name
...
Nothing in Sapote reads physical address zero. Leaving it unmapped turns a null
...
`SAPOTE_EARLY_PHYSICAL_LIMIT` is a promise that every physical address below
...
Sapote checks that entry and entry 0 rather than assuming firmware defaults. It
...
- **`CR0.WP`** (bit 16). With it clear, supervisor writes ignore the read-only
  bit entirely — and ring 0 is the only ring Sapote has, so every permission
...
4. Every table frame is below `SAPOTE_EARLY_PHYSICAL_LIMIT` and identity-mapped,
...
```text
Sapote: paging root 0x00000000001A3000 table frames 11 regions 5 NX yes write protect yes
Sapote: paging leaves 4602 writable 4545 executable 34 both 0
Sapote: kernel page tables installed
Sapote: no writable executable mapping
Sapote: virtual memory established
```
