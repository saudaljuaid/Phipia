...
OpenSeneri booted on hardware the PC has carried since 1981 and has been moving off
...
OpenSeneri's shadow copy, because what matters is what the hardware holds, not what
OpenSeneri believes it wrote.
...
live path for interrupts OpenSeneri has decided not to accept, so it is masked like
...
```text
OpenSeneri: legacy 8259 retired
OpenSeneri: timer survives legacy retirement
```
...
through hardware OpenSeneri believed it had retired.
