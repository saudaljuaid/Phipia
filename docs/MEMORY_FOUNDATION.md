...
This increment turns the boot loader's byte buffer into a validated Pyrenis
...
Boot modules are rejected until Pyrenis can assign their lifetime explicitly;
...
Pyrenis permanently removes these ranges from allocation:
...
```text
Pyrenis: memory foundation passed
```
