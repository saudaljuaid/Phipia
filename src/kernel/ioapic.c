 ...
 * owns; everything else in the low word is Seneri's to write.
 */
 ...
 * electricals Seneri cannot program, so it is refused rather than read as the
...
/* Compose the two halves of a redirection entry from what Seneri intends. */
 ...
 * IRR from software except the mask and unmask dance, which Seneri does not
...
)
{
    ...
    /*
     * Read the entry back and compare only the fields Seneri wrote. Delivery
...
enum ioapic_status ioapic_send_eoi(uint8_t vector)
{
     ...
     * entry that is level triggered in Seneri's records but edge triggered in
