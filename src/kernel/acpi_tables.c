...
/* Pyrenis early-boot policy bounds firmware-controlled work. */
 ...
 * Pyrenis requires is inside that span, so a table of exactly that size is still
...
/*
 * Two allocation structures, which is one more than any machine Pyrenis is
...
static const char test_oem_id[6] = {'P', 'Y', 'R', 'E', 'N', 'I'};
static const char test_oem_table_id[8] = {'P', 'Y', 'R', 'E', 'N', 'I', 'S', ' '};
 ...
 * Every table Pyrenis reads comes through here, so a referenced table is bounds
 ...
 *
 * This is the first optional table Pyrenis reads. A machine with no PCI Express
...
bool acpi_tables_self_test(void)
{
    ...
    root = test_root(&fixture, ACPI_ROOT_XSDT);
    root.physical_address =
        PYRENIS_EARLY_PHYSICAL_LIMIT - ACPI_SDT_HEADER_SIZE + 1U;
