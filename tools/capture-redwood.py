...
"""Capture the running Sapote Redwood from QEMU.
...
def record_live_window(args, qmp, pointer, work, output, durable_data, video,
    ...
    try:
        ...
        capture_png(qmp, work, output, "sapote-settings-all")
        pointer.move_to(600, 188)
        pointer.click()
        pointer.settle_guest(0.20)
        capture_png(qmp, work, output, "sapote-settings-dock-controls")
        ...
        capture_png(qmp, work, output, "sapote-settings-desktop")
        ...
        capture_png(qmp, work, output, "sapote-settings-appearance-light")
        pointer.move_to(654, 240)
        pointer.click()
        wait_until(started, 4.65)
        capture_png(qmp, work, output, "sapote-settings-appearance-dark")
        ...
        capture_png(qmp, work, output, "sapote-redwood-files")
        capture_png(qmp, work, output, "sapote-multitasking")
        ...
        capture_png(qmp, work, output, "sapote-redwood-notes")
        ...
        capture_png(qmp, work, output, "sapote-settings-appearance-restored")
...
def record_fast_demo(args, qmp, pointer, work, output, video, crop):
    ...
    command = [
        ...
        "-framerate", "60", "-i", "title=QEMU (SapoteCapture-0)",
        "-t", f"{args.seconds:.3f}",
    ...
    try:
        ...
        capture_png(qmp, work, output, "sapote-fast-wallpapers")
        pointer.move_to(858, 180)
        pointer.click()
...
def main():
    ...
    durable_data = output / "sapote-redwood-data.raw"
    ...
    try:
        ...
        populated = fat32_image.populate_data_files(durable_data.read_bytes(), [
            ...
            ("NEW1.TXT", b"Ready for a Sapote note."),
        ])
        fat32_image.atomic_write(durable_data, populated)
    finally:
        staged_media.unlink(missing_ok=True)
    serial = output / "sapote-redwood-serial.log"
    if serial.exists():
        serial.unlink()
    video = output / "sapote-ui-redesign-25s.mp4"
    port = free_port()
    command = [
    ...
    with tempfile.TemporaryDirectory(prefix="sapote-redwood-") as raw:
        work = Path(raw)
        ...
        try:
            ...
            time.sleep(0.35)
            pointer = Pointer(qmp)
            capture_png(qmp, work, output, "sapote-redwood")
            ...
            if args.live_window:
                ...
                while time.monotonic() - started < args.seconds + 30.0:
                    ...
                    if elapsed >= 0.10 and "dock_hover" not in events:
                        ...
                        pointer.settle_guest()
                        capture_png(qmp, work, output,
                                    "sapote-settings-appearance-light")
                        ...
                        pointer.settle_guest(0.20)
                        capture_png(qmp, work, output,
                                    "sapote-settings-appearance-restored")
                        events.update({
