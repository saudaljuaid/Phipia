...
| **Case by case** | AGPL-3.0 (combinable with GPLv3 but imposes obligations SapStudio does not want), any licence with an added patent or field-of-use clause, any dual licence whose free half is the incompatible one |
...
| `spin` | Rust | MIT | T0 | Candidate | Single-core today makes most locks unnecessary; revisit at `SAP-11`. |
...
| `fixed` | Rust | MIT / Apache-2.0 | T0 | Candidate | Fixed-point paths where exactness beats range, especially before `SAP-04`. |
...
| `rustfft` | Rust | MIT / Apache-2.0 | T2 | Watch | Needs `std` and wants SIMD; revisit after `SAP-04`. |
...
| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| `dav1d` | C + asm | BSD-2-Clause | T2 (`SAP-04`) | Candidate | The best AV1 decoder there is. Its value is its assembly, so it is worth nothing before SIMD exists. |
| `rav1d` | Rust | BSD-2-Clause | T2 (`SAP-04`) | Watch | A Rust translation of `dav1d`; watch its maturity, it would remove a C boundary entirely. |
...
| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| `rav1e` | Rust | BSD-2-Clause | T2 (`SAP-04`, `SAP-11`) | **Candidate (leading)** | Pure Rust, deterministic by design, and it removes a C boundary from the export path. |
...
| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| FFV1 | — | spec is open; FFmpeg's implementation is LGPL-2.1-or-later | T0/T1 | **Adopt (own implementation)** | Lossless, intra-only, range-coded, fully specified in RFC 9043. Writing this in safe Rust is achievable and gives SapStudio a mezzanine format it owns end to end. |
| Uncompressed planar (`SPRW`) | — | SapStudio's own | T0 | Adopt | The very first format: no entropy coding at all, so the pipeline can be proven before any codec exists. |
| ProRes (FFmpeg implementation) | C | LGPL-2.1-or-later | T2 | Watch | Reverse-engineered, and the name is Apple's trademark. Usable for ingest compatibility, never as SapStudio's own mezzanine. |
...
| `image` | Rust | MIT / Apache-2.0 | T1 | Reject | A convenience umbrella that pulls a large tree for formats SapStudio does not want. Depend on the format crates directly. |
...
| `hound` | Rust | Apache-2.0 | T1 | Adopt | WAV read and write. The first audio format SapStudio needs, and the simplest. |
...
| Fraunhofer FDK AAC | C | FDK licence | T3 | **Reject** | Licence is incompatible with GPL. This is the single most common licensing mistake in media software and SapStudio will not make it. |
...
| `fundsp` | Rust | MIT / Apache-2.0 | T1 | Watch | An elegant graph DSP library; overlaps SapStudio's own audio graph, so probably R-12.7. |
| LADSPA / LV2 hosting (`lilv`) | C | ISC | T2 (`SAP-09`) | Watch | Third-party audio plugins are a post-1.0 conversation, and they need process isolation first. |
...
| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| Colour pipeline maths | Rust | SapStudio's own | T0 | **Adopt (own)** | Primaries, transfer functions, matrices, and range handling are small, exactly specifiable, and central. R-8.2 makes them a core type, not a dependency. |
...
| `dcv-color-primitives` | Rust | MIT | T2 (`SAP-04`) | Watch | YUV↔RGB with SIMD. Value arrives with `SAP-04`. |
...
| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| Timecode | Rust | SapStudio's own | T0 | **Adopt (own)** | Drop-frame arithmetic is where editors are wrong most often. It is thirty lines of exact rational maths and a thousand lines of tests, and it belongs here (R-4.8). |
...
Everything here is blocked on `SAP-10` and `SAP-11`, and is listed so the job
graph is designed for it now rather than retrofitted later.

| Component | Language | Licence | Tier | Verdict | Notes |
| --- | --- | --- | --- | --- | --- |
| SapStudio job graph | Rust | own | T0 | Adopt (own) | Deterministic, order-independent, testable serially today and parallel later (R-6.2). |
| `rayon` | Rust | MIT / Apache-2.0 | T2 (`SAP-10`) | Watch | Work stealing makes reduction order non-deterministic unless the reduction shape is fixed. R-4.4 would have to be enforced above it. |
...
| QEMU | GPL-2.0-only (the emulator) | **Adopt (tool)** | Acceptance environment. A tool that is run, never linked — so its licence does not reach SapStudio. |
...
## 27. What SapStudio writes itself
