...
`sapstudio-abi` or `sapstudio-rt`, the only two crates R-3.1.4 permits to
contain `unsafe`.
...
**R-3.2.1** There is exactly one boundary crate, `sapstudio-abi`. Every
...
**R-3.2.9** Boundary symbols are prefixed `sapstudio_`. The kernel-facing shim
symbols are prefixed `sapstudio_sys_`. No other global symbol may be exported
...
and the same warnings-as-errors and no-SIMD-until-`SAP-04` rules as C.
...
C++ type ever appears in SapStudio's own code.
...
**R-12.7** A dependency that duplicates something SapStudio must own anyway —
the time model, the project format, the timeline — is refused. See
[`DEPENDENCIES.md`](DEPENDENCIES.md) section "What SapStudio writes itself".
