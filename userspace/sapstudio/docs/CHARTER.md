...
Sapote v1.1.0 cannot run SapStudio, and no amount of application code changes
that. Sapote has no native application ABI, no writable filesystem, no
userspace memory service, no userspace time source, no audio device, no second
core, no userspace access to the framebuffer or to input, and no architectural
guarantee that a Ring 3 program may execute a single SSE instruction. Its only
userspace surface is a deliberately bounded Linux compatibility boundary that
admits three checksum-pinned BusyBox programs and refuses everything else.

So SapStudio's first work is not a timeline. It is:
...
SapStudio does not aim at, and will refuse work toward: cross-platform support,
