...
industry uses a word loosely, SapStudio picks one meaning and keeps it.
...
**Media asset** — a file SapStudio has probed, identified by a content digest
...
for editing. SapStudio's own are `SPRW`, then FFV1.
...
SapStudio relinks by content digest, never by path alone.
...
one fades in. In SapStudio it sits at a cut rather than being an item, and it
...
stated operator. In SapStudio, always in linear light.
...
SapStudio the multiply happens **in linear light**: a premultiplied sample is
...
**Turn** — a full revolution, as a unit of angle. SapStudio's sine and cosine
