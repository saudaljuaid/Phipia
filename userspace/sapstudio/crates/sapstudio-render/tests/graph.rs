...
use sapstudio_render::{Graph, Look, Node, NodeId, RenderStatus};
...
impl sapstudio_render::Library for NoMedia {
    fn frame(
        &mut self,
        _media: sapstudio_core::Digest,
        _tick: i64,
    ...
    fn look(&mut self, _look: sapstudio_core::Digest) -> Result<Look, RenderStatus> {
        Err(RenderStatus::UnknownNode)
    }
}
...
fn a_node_may_only_name_nodes_that_already_exist_on_every_input() {
    ...
    assert_eq!(
        graph.clone().add(Node::Fade {
            input: ghost,
            opacity: sapstudio_core::Rational::ONE,
        }),
        Err(RenderStatus::UnknownNode)
    );
...
fn a_faded_node_is_a_different_node_at_every_opacity() {
    ...
    for step in 1..6 {
        let node = graph
            .add(Node::Fade {
                input: bars,
                opacity: sapstudio_core::Rational::new(step, 6).expect("a ratio"),
            })
            .expect("a node");
