...
use sapstudio_render::shape::{Edge, FULL, MAX_EDGES, Shape, half_plane_coverage, plane, quantise};
...
fn a_wipe_at_nothing_covers_nothing_and_at_all_covers_everything() {
    ...
    // shaped to avoid.
    use sapstudio_render::shape::sweeping;
...
fn no_softness_is_the_hard_edge_exactly() {
    ...
    use sapstudio_render::shape::{feathered, sweeping};
    let hard = plane(
...
fn a_soft_edge_and_its_complement_still_partition_every_pixel() {
    ...
    // along the edge.
    use sapstudio_render::shape::Feather;
    let edge = Edge::new(n(3), n(-7), r(23, 2)).expect("an edge");
...
fn a_softness_past_its_own_range_is_refused() {
    use sapstudio_render::shape::feathered;
    assert_eq!(
...
fn a_mask_covers_the_fraction_of_the_frame_it_encloses() {
    ...
    use sapstudio_render::shape::masking;
