...
use sapstudio_render::{RenderStatus, chroma_of, vectorscope};
...
fn rgb(width: u32, height: u32, format: PixelFormat) -> FrameDescription {
    FrameDescription::square(
        ...
        None,
        if format.has_alpha() {
            Some(sapstudio_media::AlphaState::Straight)
        } else {
            None
        },
    )
...
fn the_grid_is_bounded_and_says_so() {
    ...
    assert_eq!(
        vectorscope(&frame, sapstudio_render::MAX_BINS + 1),
        Err(RenderStatus::OutsideDomain)
    );
    let scope = vectorscope(&frame, sapstudio_render::MAX_BINS).expect("a scope");
    assert_eq!(scope.bins(), sapstudio_render::MAX_BINS);
    assert_eq!(
        scope.count(sapstudio_render::MAX_BINS, 0),
        Err(RenderStatus::OutsideDomain)
    );
}
