...
fn pieces_of(character: char, size: i64) -> Vec<Shape> {
    ...
    for piece in &glyph.pieces().expect("pieces") {
        let corners: Vec<(Rational, Rational)> = piece
            ...
            .collect();
        out.push(sapstudio_render::shape::convex(&corners).expect("a shape"));
    }
    out
}
...
fn placing_a_run_somewhere_else_moves_it_and_nothing_more() {
    let face = face();
    let size = r(12, 1);
    let here = font::place(face, "SAP", size, (Rational::ZERO, Rational::ZERO))
        .expect("a run")
        .plane(60, 30)
        .expect("a plane");
    let there = font::place(face, "SAP", size, (r(7, 1), r(5, 1)))
        .expect("a run")
        .plane(60, 30)
        .expect("a plane");
    for y in 0..25 {
...
fn the_face_sets_a_name() {
    ...
    // a face for slates.
    let face = face();
    for words in ["Sap Studio", "The End", "Directed by"] {
