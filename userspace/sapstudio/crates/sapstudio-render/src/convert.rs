...
use sapstudio_media::{ColourDescription, Frame, FrameDescription, PixelFormat, Range};
...
pub(crate) fn quantise(value: Fixed, range: Range, chroma: bool) -> Result<u8> {
    ...
    let rounded = scaled.checked_add(half)?.raw() >> sapstudio_core::FRACTION_BITS;
...
pub fn convert(frame: &Frame, target: FrameDescription) -> Result<Frame> {
    ...
    // `SAP-03`'s problem before it is this function's.
