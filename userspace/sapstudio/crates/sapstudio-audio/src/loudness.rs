...
use sapstudio_core::{Fixed, Rational, WIDE_BITS};
...
impl Biquad {
    ...
    fn step(&mut self, input: i128) -> Result<i128> {
        ...
        // option.
        let overflow = AudioStatus::Time(sapstudio_core::CoreStatus::Overflow);
...
fn block_powers(buffer: &AudioBuffer) -> Result<Vec<i128>> {
        ...
        for (index, sample) in samples.iter().enumerate() {
            ...
            if let Some(slot) = energy.get_mut(step) {
                *slot = slot
                    ...
                    .ok_or(AudioStatus::Time(sapstudio_core::CoreStatus::Overflow))?;
            }
    ...
    let divisor = i128::try_from(BLOCK_SAMPLES)
        .map_err(|_| AudioStatus::Time(sapstudio_core::CoreStatus::Overflow))?;
...
fn narrow_to_fixed(value: i128) -> Result<i64> {
    let shift = WIDE_BITS - sapstudio_core::FRACTION_BITS;
    ...
    i64::try_from(rounded).map_err(|_| AudioStatus::Time(sapstudio_core::CoreStatus::Overflow))
}
