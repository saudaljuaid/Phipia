...
//! drop-frame timecode. There is no picture yet, because `SAP-06` does not
...
use sapstudio_abi::seam::{Console, ConsoleWriter, SeamStatus};
use sapstudio_core::{CoreStatus, Duration, Instant, Timebase, Timecode};
use sapstudio_io::sprw::{self, Reel};
use sapstudio_io::{IoStatus, MemoryStorage};
use sapstudio_media::{
...
use sapstudio_model::media::Digest;
use sapstudio_model::{Clip, Edit, Item, MediaAsset, ModelStatus, Project, TrackKind};
...
pub enum SlateStatus {
    ...
    Render(sapstudio_render::RenderStatus),
    ...
    Audio(sapstudio_audio::AudioStatus),
}

impl From<sapstudio_audio::AudioStatus> for SlateStatus {
    fn from(status: sapstudio_audio::AudioStatus) -> Self {
        Self::Audio(status)
    }
}

impl From<sapstudio_render::RenderStatus> for SlateStatus {
    fn from(status: sapstudio_render::RenderStatus) -> Self {
        Self::Render(status)
    }
}
...
pub fn run(console: &mut dyn Console) -> i32 {
    match slate(console) {
        ...
        Err(failure) => {
            ...
            let _ = writeln!(writer, "sapstudio: refused: {failure:?}");
            EXIT_FAILURE
        }
    }
}
...
pub fn slate(console: &mut dyn Console) -> Result<(), SlateStatus> {
    ...
    let digest = sapstudio_io::save(&project, &mut storage)?;
    let bytes = storage.committed().map_or(0, <[u8]>::len);
    let reloaded = sapstudio_io::load(&storage)?;
    ...
    let again = sapstudio_io::save(&reloaded, &mut storage)?;
...
/// freestanding target with the heap it actually has. `SAP-03` is what turns
/// this into a frame.
fn reel(console: &mut dyn Console) -> Result<(), SlateStatus> {
    ...
    say(
        console,
        "reel digest    ",
        sapstudio_core::Digest::of(&encoded),
    )?;
...
/// media. A real one reads a file; `SAP-08` is what turns this into that.
struct Held {
    frames: alloc::vec::Vec<(Digest, sapstudio_media::Frame)>,
}

impl sapstudio_render::Library for Held {
    fn frame(
        &mut self,
        media: Digest,
        _tick: i64,
        description: FrameDescription,
    ) -> Result<sapstudio_media::Frame, sapstudio_render::RenderStatus> {
        let held = self
            .frames
            .iter()
            .find(|(digest, _)| *digest == media)
            .map(|(_, frame)| frame.clone())
            .ok_or(sapstudio_render::RenderStatus::UnknownNode)?;
    ...
    fn look(
        &mut self,
        _look: Digest,
    ) -> Result<sapstudio_render::Look, sapstudio_render::RenderStatus> {
        // Nothing here is graded. A request for a look is a fault rather than
        // a case to handle, and refusing says so.
        Err(sapstudio_render::RenderStatus::UnknownNode)
    }
}
...
/// project is made of.
///
/// Sixteen by nine pixels, which is not a picture. `SAP-03` is what turns it
/// into one.
///
...
fn picture(console: &mut dyn Console) -> Result<(), SlateStatus> {
    ...
    let rise = Curve::new({
        ...
        keyframes.push(Keyframe::new(
            Instant::new(0, RATE),
            sapstudio_core::Rational::new(0, 1)?,
            Interpolation::Linear,
        )?);
        keyframes.push(Keyframe::new(
            Instant::new(24, RATE),
            sapstudio_core::Rational::new(1, 1)?,
            Interpolation::Hold,
        )?);
        keyframes
    })?;
...
fn flat(
    description: FrameDescription,
    pixel: [u8; 4],
) -> Result<sapstudio_media::Frame, SlateStatus> {
    ...
    Ok(sapstudio_media::Frame::from_packed(description, &bytes)?)
}
...
fn insert_clip(
    project: &mut Project,
    sequence: sapstudio_model::SequenceId,
    media: sapstudio_model::MediaId,
    track: usize,
    index: usize,
    source_start: i64,
    length: i64,
