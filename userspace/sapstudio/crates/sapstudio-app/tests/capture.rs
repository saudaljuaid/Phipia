...
use sapstudio_media::colour::{MatrixCoefficients, Primaries, Range, TransferFunction};
use sapstudio_media::{
    AlphaState, ColourDescription, Frame, FrameDescription, Geometry, PixelFormat, TestPattern,
    pool::FramePool,
};
use sapstudio_model::{Clip, Edit, Item, Mask, MediaAsset, Project, TrackKind, Transition, Wipe};
...
fn composed() -> (Project, sapstudio_model::SequenceId) {
...
fn dress(project: &mut Project, sequence: sapstudio_model::SequenceId) {
...
fn the_reference_capture_is_the_picture_beside_it() {
    ...
    let frame = sapstudio_app::timeline::render(
        &project,
        sequence,
        Instant::new(21, RATE),
        described(),
    ...
    let straight = sapstudio_render::composite::unpremultiply(&frame).expect("a straight frame");
    let captured = sapstudio_io::png::encode(&straight).expect("a capture");
