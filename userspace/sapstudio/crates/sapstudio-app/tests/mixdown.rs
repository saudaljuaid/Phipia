...
fn a_source_that_answers_with_a_different_span_is_refused() {
    ...
    assert_eq!(
        ...
        Err(SlateStatus::Audio(sapstudio_audio::AudioStatus::NotMixable))
    );
}
...
fn the_two_fader_bounds_agree() {
    ...
    assert_eq!(
        sapstudio_model::MINIMUM_DECIBELS,
        sapstudio_audio::MINIMUM_DECIBELS
    );
    assert_eq!(
        sapstudio_model::MAXIMUM_DECIBELS,
        sapstudio_audio::MAXIMUM_DECIBELS
    );
}
...
fn mixed_with(
    ...
    frames: i64,
    sample: i32,
) -> (AudioBuffer, sapstudio_audio::MixReport) {
...
fn a_track_with_no_automation_still_reads_its_static_fader() {
    ...
    project
        .apply(
            ...
            Edit::SetTrackFader {
                track: 0,
                fader: sapstudio_model::Fader::at(Rational::new(-20, 1).expect("a value"))
                    .expect("a fader"),
            },
        )
        .expect("a fader");
...
fn a_muted_track_stays_muted_under_automation() {
    ...
    project
        .apply(
            ...
            Edit::SetTrackFader {
                track: 0,
                fader: sapstudio_model::Fader::MUTED,
            },
        )
        .expect("a mute");
