...
use sapstudio_core::{CoreStatus, Instant, Timebase, Timecode};
...
fn drop_frame_labels_track_wall_clock_time() {
    ...
    let one_hour = sapstudio_core::Rational::from_integer(3600);
    let drift = seconds.checked_sub(one_hour).expect("a difference");
    // |drift| < 1/30 of a second.
    let bound = sapstudio_core::Rational::new(1, 30).expect("a frame of time");
