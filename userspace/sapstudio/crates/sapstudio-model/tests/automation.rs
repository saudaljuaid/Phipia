...
fn project() -> (Project, sapstudio_model::SequenceId) {
    let mut project = Project::new();
    let media = project
        .add_media(
            MediaAsset::new(
                sapstudio_model::Digest::of(b"one"),
                RATE,
...
fn automation_multiplies_a_dissolve_rather_than_replacing_it() {
    ...
    project
        .apply(
            ...
            Edit::AddTransition {
                track: 0,
                transition: sapstudio_model::Transition::new(
                    1,
...
fn an_automated_fader_saturates_at_the_ends_of_its_travel() {
    ...
    assert_eq!(
        project
            ...
            .expect("a fader")
            .decibels(),
        Some(value(sapstudio_model::MINIMUM_DECIBELS, 1)),
