...
fn a_card_may_say_more_than_one_thing() {
    let card = Title::new(
        std::vec!["Sap Studio".into(), "a cut that keeps".into()],
        r(1, 8),
        r(1, 2),
        r(1, 2),
        sapstudio_model::Alignment::Left,
    )
    .expect("a title");
    assert_eq!(card.lines().len(), 2);
    assert_eq!(card.alignment(), sapstudio_model::Alignment::Left);
}
...
fn a_blank_line_among_others_is_how_a_card_puts_air_in() {
    ...
    assert!(
        Title::new(
            ...
            r(1, 2),
            r(1, 2),
            sapstudio_model::Alignment::Centre,
        )
        .is_ok()
    );
}
...
fn a_card_of_more_lines_than_this_describes_is_refused() {
    use sapstudio_model::MAX_TITLE_LINES;
