...
use sapstudio_model::{Clip, Edit, Item, Lane, MediaAsset, ModelStatus, Project, TrackKind};
...
fn an_instant_in_another_timebase_is_refused() {
    ...
    assert_eq!(
        ...
        Err(ModelStatus::Time(
            sapstudio_core::CoreStatus::TimebaseMismatch
        ))
    );
}
...
fn the_fader_has_ends() {
    use sapstudio_core::Rational;
    use sapstudio_model::{Fader, MAXIMUM_DECIBELS, MINIMUM_DECIBELS};
...
fn two_clips(
    project: &mut Project,
    sequence: sapstudio_model::SequenceId,
    media: [sapstudio_model::MediaId; 3],
    each: i64,
    handle: i64,
) {
    lay(
...
fn a_dissolve_needs_a_cut_between_two_clips() {
    ...
    assert_eq!(
        Transition::new(1, sapstudio_core::Duration::zero(RATE)),
...
fn a_wipe_with_no_direction_is_refused() {
    use sapstudio_core::Rational;
    use sapstudio_model::{ModelStatus, Wipe};
    assert_eq!(
