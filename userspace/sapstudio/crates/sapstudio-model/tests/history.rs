...
use sapstudio_core::{Duration, Timebase};
...
fn fresh() -> (Project, MediaId, sapstudio_model::SequenceId) {
...
fn invent(generator: &mut Generator, sequence: &Sequence, media: MediaId) -> Edit {
    ...
    let items = sequence.track(track).map_or(0, sapstudio_model::Track::len);
