...
use sapstudio_core::{Instant, Rational, Timebase};
use sapstudio_model::ModelStatus;
use sapstudio_model::curve::{Curve, EASE_BITS, Interpolation, Keyframe, MAX_KEYFRAMES};
...
fn arithmetic_that_will_not_fit_is_refused_rather_than_wrapped() {
    ...
    assert_eq!(
        ...
        Err(ModelStatus::Time(sapstudio_core::CoreStatus::Overflow))
    );
}
