...
//! and reusing it means `SAP-01` is a new *numbering and allowlist*, not new
//! assembly.
//!
...
#[repr(u64)]
pub enum Number {
    /// `SAP-01`. Stop, with a status. Does not return.
    Exit = 0,
    /// `SAP-01`. Write bytes to the kernel's console and serial transcript.
    ConsoleWrite = 1,
    /// `SAP-05`. Nanoseconds from the kernel's one monotonic source.
    MonotonicNanoseconds = 2,
    /// `SAP-03`. Map anonymous read-write, no-execute pages.
    MapAnonymous = 3,
    /// `SAP-03`. Release pages a previous call mapped.
    UnmapAnonymous = 4,
    /// `SAP-06`. Obtain a pixel surface of a fixed geometry.
    SurfaceAcquire = 5,
    /// `SAP-06`. Ask the kernel to present a damage rectangle from it.
    SurfacePresent = 6,
    /// `SAP-07`. Drain the bounded input event queue.
    InputDrain = 7,
}
