...
use sapstudio_audio::overview::{Bucket, Overview};
use sapstudio_audio::{MAX_CHANNELS, SampleRate};
...
pub fn decode(file: &[u8]) -> Result<Overview> {
    ...
    if channels == 0 || channels > MAX_CHANNELS {
        return Err(IoStatus::Sound(
            sapstudio_audio::AudioStatus::ChannelCountUnsupported,
        ));
    }
    if header.u16()? != 0 {
        return Err(IoStatus::ReservedFieldSet);
...
fn shape_of(length: usize, base: usize, channels: usize) -> Result<Vec<usize>> {
    if length == 0 {
        return Err(IoStatus::Sound(
            sapstudio_audio::AudioStatus::BufferTooShort,
        ));
    }
    sapstudio_audio::overview::check_base(base)?;
    if length > sapstudio_audio::MAX_SAMPLES {
        return Err(IoStatus::Sound(sapstudio_audio::AudioStatus::BufferTooLong));
    }
    ...
    loop {
        ...
        count = count.div_ceil(sapstudio_audio::overview::FANOUT);
    }
    Ok(shape)
}
