...
use sapstudio_abi::seam::{SeamStatus, Slot, Storage};
use sapstudio_core::{Digest, Duration, Timebase};
use sapstudio_io::{Fault, IoStatus, MemoryStorage, encode, load, save};
...
fn load_from_bytes(bytes: &[u8]) -> Project {
    sapstudio_io::decode(bytes).expect("the committed bytes are a project")
}
