...
use sapstudio_core::{Digest, Duration, Instant, Rational, Timebase, Timecode};
use sapstudio_io::conform::{self, LeftBehind};
use sapstudio_io::edl;
use sapstudio_io::{Channel, IoStatus, Transition};
use sapstudio_model::curve::{Curve, Interpolation, Keyframe};
use sapstudio_model::track::Fader;
use sapstudio_model::{
...
fn hand_built(events: Vec<sapstudio_io::Event>) -> sapstudio_io::EditDecisionList {
...
fn conforming_is_not_editing() {
    ...
    assert_eq!(
        into.undo(landed),
        Err(sapstudio_model::ModelStatus::NothingToDo),
...
fn a_list_read_at_the_wrong_rate_is_caught_by_the_media_and_not_by_the_file() {
    ...
    assert_eq!(
        ...
        Err(IoStatus::Model(
            sapstudio_model::ModelStatus::MediaTimebaseMismatch
        )),
...
fn a_label_that_cannot_count_at_the_told_rate_is_refused() {
    ...
    assert_eq!(
        conform::import(&mut project, &list, RATE, start()),
        Err(IoStatus::Time(
            sapstudio_core::CoreStatus::TimecodeMalformed
        ))
    );
}
...
fn a_wipe_is_left_behind_and_comes_back_a_dissolve() {
        ...
        .apply(
            ...
            Edit::AddTransition {
                ...
                transition: track::Transition::wiping(
                    held.boundary(),
                    held.duration(),
                    sapstudio_model::Wipe::RIGHTWARD,
                )
                .expect("a wipe"),
            },
        )
    ...
    assert_eq!(
        into.sequence(landed)
            ...
            .transitions()[0]
            .kind(),
        sapstudio_model::TransitionKind::Dissolve,
        "a reported loss that changed nothing would be a warning nobody should heed"
    );
    assert_ne!(
        into.sequence(landed).expect("the sequence"),
