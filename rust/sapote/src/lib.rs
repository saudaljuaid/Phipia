...
unsafe fn syscall0(number: u64) -> i64 {
    let result: i64;
    // SAFETY: the Sapote ABI declares RCX and R11 clobbered by SYSCALL.
    unsafe {
...
#[macro_export]
macro_rules! sapote_main {
    ($main:path) => {
        #[unsafe(no_mangle)]
        pub extern "C" fn sapote_rust_start(argc: usize, argv: *const *const u8,
            environment: *const *const u8) -> ! {
...
fn panic(_information: &PanicInfo<'_>) -> ! {
    let _ = console_write(b"Sapote Rust application panic\n");
    exit(127)
}
