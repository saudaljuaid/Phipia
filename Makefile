	...
	case '$*' in \
		...
		boot-ledger) \
			...
			grep -Fxq 'Sapote: Boot Ledger installed proof passed' "$$log" || \
				diagnostics_ok=false ;; \
		redwood-proof) \
		grep -Eq '^ST REDWOOD_PROOF geometry 1024x768 dock 8 events [1-9][0-9]* panels [4-9][0-9]* cursor [1-9][0-9]* damage [1-9][0-9]* glyphs [1-9][0-9]* fingerprint 0x[0-9A-F]{16}$$' "$$log" && \
